...
class _EditProductPageState extends State<EditProductPage> {
  ...
  int calculateMaxProducts() { ///
    ...
    LinkedHashSet<Item> tempInventory = LinkedHashSet<Item>.from(
      widget.inventory.getItems().map((item) => item.duplicate()),
    ); // deep copy
    while (true) {
                                        ...
                                        showDialog(
                                          ...
                                          builder: (BuildContext context) {
                                            return AlertDialog(
                                              contentPadding: EdgeInsets.symmetric(vertical: 20, horizontal: 20),
                                              title: Text('Stocks'),
                                              content: Text('If a component is selected from inventory, the entered quantity must not exceed available stock. Otherwise, the product will be added as a new item to inventory and the entered quantity will be reflected.'), ///
                                              actions: [
                                                TextButton(
                                                  onPressed: () {
                                                    Navigator.of(context).pop();
